...
const logger = (0, logger_1.createLogger)('ResolverAPI');
function setupRoutes(app, resolver) {
    ...
    app.get('/api/orders/:orderHash', async (req, res) => {
        try {
            const { orderHash } = req.params;
            logger.info(`📊 Status request for order: ${orderHash.slice(0, 10)}`);
            // Check if order is active
            const resolverStatus = resolver.getStatus();
            const isActive = resolverStatus.activeOrders.some(hash => hash === orderHash.slice(0, 10));
            if (isActive) {
                res.json({
                    orderHash,
                    status: 'executing',
    ...
    app.get('/api/resolver/stats', (req, res) => {
        try {
    ...
    app.get('/api/resolver/orders', (req, res) => {
        try {
    ...
    app.get('/api/resolver/config', (req, res) => {
        try {
            res.json({
                supportedChains: {
    ...
    app.post('/api/resolver/process/:orderHash', async (req, res) => {
        try {
            const { orderHash } = req.params;
