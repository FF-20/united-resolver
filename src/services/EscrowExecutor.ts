...
const logger = createLogger('EscrowExecutor');
...
export class EscrowExecutor {
  ...
  private async initializeEthereumConnections(): Promise<void> {
  ...
  private async initializeCosmosConnections(): Promise<void> {
    ...
    this.cosmosWallet = await DirectSecp256k1HdWallet.fromMnemonic(mnemonic, {
      prefix: 'cosmos'
    });

    this.cosmosClient = await SigningStargateClient.connectWithSigner(
      process.env.COSMOS_RPC_URL || 'https://rpc-cosmoshub.blockapsis.com',
      this.cosmosWallet
    );
  ...
  async lockSourceEscrow(order: CrossChainOrder, decision: ResolverDecision): Promise<TransactionResult> {
    try {
  ...
  async lockDestinationEscrow(order: CrossChainOrder, decision: ResolverDecision): Promise<TransactionResult> {
    try {
  ...
  private async lockEvmEscrow(order: CrossChainOrder, type: 'source' | 'destination'): Promise<TransactionResult> {
    ...
    const escrowContract = new ethers.Contract(escrowAddress, escrowABI, this.ethereumSigner);
    ...
    const tx = await escrowContract.lockFunds(
      ...
      timelock,
      type === 'source' ? await this.ethereumSigner.getAddress() : order.maker,
      {
        value: token === ethers.ZeroAddress ? ethers.parseUnits(amount, 18) : 0,
        gasLimit: 300000
      }
    );
  ...
  private async lockCosmosEscrow(order: CrossChainOrder, type: 'source' | 'destination'): Promise<TransactionResult> {
    const amount = type === 'source' ? order.srcAmount : order.dstAmount;
    ...
    const msg = {
      ...
      value: {
        ...
        amount: [{
          denom: denom,
          amount: ethers.parseUnits(amount, 6).toString() // Cosmos uses 6 decimals
        }]
      }
    };
    ...
    const result = await this.cosmosClient.signAndBroadcast(
      ...
      [msg],
      fee,
      `Escrow lock for order ${order.orderHash.slice(0, 10)}`
    );
  ...
  private async approveToken(tokenAddress: string, spender: string, amount: string): Promise<void> {
    ...
    const tokenContract = new ethers.Contract(tokenAddress, tokenABI, this.ethereumSigner);
    
    // Check current allowance
    const currentAllowance = await tokenContract.allowance(
      await this.ethereumSigner.getAddress(),
      spender
    );
  ...
  async revealSecretAndClaim(order: CrossChainOrder, decision: ResolverDecision): Promise<void> {
  ...
  private async claimFromEscrow(order: CrossChainOrder, secret: Uint8Array, type: 'source' | 'destination'): Promise<void> {
    const chainId = type === 'source' ? order.srcChainId : order.dstChainId;
  ...
  private async claimFromEvmEscrow(order: CrossChainOrder, secret: Uint8Array, type: 'source' | 'destination'): Promise<void> {
    const chainId = type === 'source' ? order.srcChainId : order.dstChainId;
    const escrowAddress = this.getEscrowAddress(chainId);
    ...
    const tx = await escrowContract.claimFunds(ethers.hexlify(secret), {
      gasLimit: 200000
    });
  ...
  private async claimFromCosmosEscrow(order: CrossChainOrder, secret: Uint8Array, type: 'source' | 'destination'): Promise<void> {
    // Implementation for claiming from Cosmos HTLC
    logger.info(`✅ Claimed from Cosmos escrow (simulated)`);
  }

  async isTransactionConfirmed(txHash: string, chain: 'source' | 'destination'): Promise<boolean> {
    try {
  ...
  async cancelEscrows(order: CrossChainOrder): Promise<void> {
  ...
  private async cancelEvmEscrow(order: CrossChainOrder, type: 'source' | 'destination'): Promise<void> {
    const chainId = type === 'source' ? order.srcChainId : order.dstChainId;
    const escrowAddress = this.getEscrowAddress(chainId);
  ...
  private getEscrowAddress(chainId: string | number): string {
    const addresses = {
