...
import { CrossChainOrder, OrderStatus, ResolverDecision } from '../types';

const logger = createLogger('CrossChainResolver');

export class CrossChainResolver {
  private isRunning = false;
  private processedOrders = new Set<string>();
  private activeOrders = new Map<string, CrossChainOrder>();
  ...
  private async analyzeAndExecuteOrder(order: CrossChainOrder): Promise<void> {
    try {
  ...
  private async executeAtomicSwap(order: CrossChainOrder, decision: ResolverDecision): Promise<void> {
  ...
  private async waitForEscrowConfirmations(sourceTxHash: string, destTxHash: string): Promise<void> {
  ...
  private async handleFailedSwap(order: CrossChainOrder, error: any): Promise<void> {
